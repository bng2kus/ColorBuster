...
// Call this method whenever you want to update the boardView on the display from the current status of the board
	...
	public int processTouchedTile(TileView tv) {
		...
		HashSet<Tile> match = new HashSet<Tile>();
		//calls locateNeighbors from Board to get match hashset
		b.locateNeighbors(tv.getRow(), tv.getCol(), tv.getColor(), match);
		if(match.size() >= 3){
			b.removeMatchingTiles(match);
			b.collapseColumns();
